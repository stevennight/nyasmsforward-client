...
void main() {
  ...
  group('starting', () {
    ...
    test(
      'a stored token is checked, the lists load and the stream opens',
      () async {
        await tokens.write('nsf_tok');
        store.value = const ClientSettings(
          serverUrl: 'https://sms.example.com',
          deviceName: '台式机',
        );
        server.routes.addAll(
          connectedRoutes(conversations: [convJson(incoming, unread: 1)]),
        );
        ...
        expect(
          ...
          'c1',
          reason: 'the identity is remembered',
        );
    ...
    test(
      'offline at launch: the app still opens and the token is kept',
      () async {
        await tokens.write('nsf_tok');
        store.value = const ClientSettings(
          serverUrl: 'https://sms.example.com',
        ...
        await until(
          () =>
              server.openStreams.isNotEmpty || server.streamRequests.isNotEmpty,
          why: 'it keeps trying to connect',
        );
      },
    );
    ...
    test('a revoked token goes back to the connect screen, keeps the address and says why', () async {
      ...
      expect(
        store.value.serverUrl,
        'https://sms.example.com',
        reason: 'the address survives sign-out',
      );
    ...
    test('a bare 401 from a proxy at launch does NOT sign out', () async {
      ...
      server.routes['GET /api/v1/conversations'] = (_) =>
          (401, '<html>proxy auth</html>');
      server.routes['GET /api/v1/phones'] = (_) =>
          (401, '<html>proxy auth</html>');
    ...
    test(
      'without the read permission nothing is streamed and the reason is shown',
      () async {
        await tokens.write('nsf_tok');
        store.value = const ClientSettings(
          serverUrl: 'https://sms.example.com',
  ...
  group('connecting', () {
    test('a pairing code becomes a stored token and a live app', () async {
      ...
      server.routes['POST /api/v1/pair/claim'] = (_) => {
        'deviceId': 'c1',
        'token': 'nsf_new',
      ...
      expect(
        server.called('POST', '/api/v1/pair/claim').single.body,
        containsPair('platform', 'windows'),
      );
      expect(
        server.called('GET', '/api/v1/me').single.headers['Authorization'],
        'Bearer nsf_new',
      );
    });

    test(
      'account login works the same way, and the password is not kept anywhere',
      () async {
        server.routes.addAll(connectedRoutes());
        server.routes['POST /api/v1/auth/login-device'] = (_) => {
          'deviceId': 'c1',
          'token': 'nsf_new',
          'scopes': ['read'],
        };
  ...
  group('live', () {
    Future<void> connected({
      ...
      Map<String, Object? Function(Req)>? extra,
    }) async {
      await tokens.write('nsf_tok');
      store.value = const ClientSettings(serverUrl: 'https://sms.example.com');
      server.routes.addAll(
        connectedRoutes(
          conversations: conversations ?? [convJson(incoming, unread: 1)],
          messages: [incoming],
        ),
      );
    ...
    test('a message the server says to alert about alerts once and refreshes the lists', () async {
      ...
      await until(
        () => server.called('GET', '/api/v1/conversations').length > listings,
        why: 'the list is refetched',
      );
      ...
      server.emit('message', {
        ...msgJson(4, direction: 'out', origin: 'platform'),
        'notify': true,
      }, id: 4);
      await until(
        () => server.called('GET', '/api/v1/conversations').length > listings,
      ...
      expect(
        alerts,
        hasLength(1),
        reason: 'replies we sent and history never alert',
      );
    });

    test(
      'a fresh connection refetches what happened before it existed',
      () async {
        await connected();
        ...
        await until(
          () => server.called('GET', '/api/v1/conversations').length > before,
        );
      },
    );
    ...
    test(
      'opening a conversation loads it, marks it read and shows its send tasks',
      () async {
        await connected(
          extra: {
            'GET /api/v1/outbound': (_) => {
              'items': [taskJson(status: 'sent')],
            },
          },
        );
        ...
        final read =
            server.called('POST', '/api/v1/messages/read').last.body as Map;
  ...
  group('sending', () {
    ...
    test('a reply names the latest received message', () async {
      ...
      expect(server.called('POST', '/api/v1/outbound').single.body, {
        'replyToMessageId': 2,
        'body': 'TD',
      });
    });
    ...
    test(
      'why a reply is not possible, most fundamental reason first',
      () async {
    ...
    test(
      'the phone policy blocks replying, and says whose policy it is',
      () async {
        await open(
          phone: {
            'effectivePolicy': 'off',
    ...
    test(
      'a refused send comes back as words and does not sign anyone out',
      () async {
        await open();
        server.routes['POST /api/v1/outbound'] = (_) =>
    ...
    test('a task that has not reached the phone can be cancelled', () async {
      await open();
      server.routes['POST /api/v1/outbound/t_1/cancel'] = (_) =>
          taskJson(status: 'failed', error: 'cancelled');
      expect(await controller.cancelTask('t_1'), isNull);
      server.routes['POST /api/v1/outbound/t_2/cancel'] = (_) =>
  ...
  group('settings', () {
    ...
    test(
      'a new address is only saved when that server knows this device',
      () async {
        // Same device answers: saved, no re-login.
        expect(
          await controller.changeServerUrl('https://sms2.example.com/'),
          isNull,
        );
        ...
        expect(
          await tokens.read(),
          'nsf_tok',
          reason: 'the token does not depend on the address',
        );
      },
    );
    ...
    test('an address of a different server, a typo, and a public http address are refused without saving', () async {
      ...
      expect(
        await controller.changeServerUrl('https://other.example.com'),
        contains('不认识'),
      );
      expect(
        await controller.changeServerUrl('http://public.example.com'),
        contains('HTTPS'),
      );
    ...
    test('an address where the token is rejected explicitly is refused with a re-connect hint', () async {
      ...
      expect(
        ...
        AppPhase.ready,
        reason:
            'a probe of the wrong server must not sign us out of the right one',
      );
    });
    ...
    test(
      'settings survive being saved and loaded, including quick replies',
      () async {
        ...
        expect(loaded.foregroundNotifications, isTrue);
        expect(ClientSettings.fromJson(loaded.toJson()).quickReplies, [
          '好的',
          'TD',
        ]);
        expect(
          ClientSettings.fromJson(loaded.toJson()).foregroundNotifications,
          isTrue,
        );
      },
    );
