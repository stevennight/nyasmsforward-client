...
class Rig {
  ...
  Future<void> pump(WidgetTester tester, {Size size = const Size(1200, 800)}) async {
...
void main() {
  group('the app root', () {
    testWidgets('without a stored connection it shows the connect form, with the remembered address after sign-out', (tester) async {
      final rig = Rig(connected: false);
      rig.store.value = const ClientSettings(serverUrl: 'https://sms.example.com', deviceName: '旧电脑');
      await rig.pump(tester);
      expect(find.text('连接到服务器'), findsOneWidget);
      expect(tester.widget<TextField>(byKey('serverUrl')).controller!.text, 'https://sms.example.com');
      expect(tester.widget<TextField>(byKey('deviceName')).controller!.text, '旧电脑');
    });

    testWidgets('connecting through the form ends in the app', (tester) async {
      final rig = Rig(connected: false);
      rig.server.routes['POST /api/v1/pair/claim'] = (_) => {'deviceId': 'c1', 'token': 'nsf_new', 'scopes': ['read', 'reply', 'send']};
      await rig.pump(tester);
    ...
    testWidgets('a refused pairing shows the reason under the form', (tester) async {
      final rig = Rig(connected: false);
      rig.server.routes['POST /api/v1/pair/claim'] = (_) => (400, {'error': 'invalid_pair_code'});
      await rig.pump(tester);
      await tester.enterText(byKey('serverUrl'), 'https://sms.example.com');
    ...
    testWidgets('a pasted pairing link fills the address and the code', (tester) async {
      final rig = Rig(connected: false);
      await rig.pump(tester);
      await tester.enterText(byKey('serverUrl'), 'nyasmsforward://pair?server=https%3A%2F%2Fsms.example.com&code=483920');
      await tester.pumpAndSettle();
      expect(tester.widget<TextField>(byKey('serverUrl')).controller!.text, 'https://sms.example.com');
      expect(tester.widget<TextField>(byKey('pairingCode')).controller!.text, '483920');
    });

    test('pairing links are parsed strictly', () {
      expect(PairLink.tryParse('nyasmsforward://pair?server=https://sms.example.com&code=483920')?.code, '483920');
      expect(PairLink.tryParse('nyasmsforward://pair?server=https%3A%2F%2Fsms.example.com&code=483%20920')?.code, '483920');
      expect(PairLink.tryParse('https://evil.example/pair?server=x&code=483920'), isNull);
      expect(PairLink.tryParse('nyasmsforward://pair?server=https://x.example&code=12'), isNull);
  ...
  group('conversations', () {
    testWidgets('a wide window shows the list and the open conversation side by side, with the code card', (tester) async {
      final rig = Rig();
      await rig.pump(tester);
    ...
    testWidgets('copying the verification code puts it on the clipboard', (tester) async {
      String? copied;
    ...
    testWidgets('on a phone the list and the conversation are separate pages and Back returns to the list', (tester) async {
      final rig = Rig();
    ...
    testWidgets('search narrows the list and an empty result is explained', (tester) async {
      final rig = Rig(extraConversations: [convJson(msgJson(9, peer: '13800000000', body: '你到家了吗？'))]);
      await rig.pump(tester);
    ...
    testWidgets('connection trouble is shown above the list, and the live indicator says so', (tester) async {
      final rig = Rig();
  ...
  group('replying', () {
    ...
    testWidgets('typing and sending posts the reply naming the message, then clears the box', (tester) async {
      final rig = Rig();
      ...
      await tester.pumpAndSettle();
      expect(rig.server.called('POST', '/api/v1/outbound').single.body, {'replyToMessageId': 2, 'body': 'TD'});
      expect(tester.widget<TextField>(byKey('replyField')).controller!.text, isEmpty);
    });
    ...
    testWidgets('a quick reply fills the box and does not send by itself', (tester) async {
      final rig = Rig();
      await rig.pump(tester);
      await openThread(tester);
      await tester.tap(find.widgetWithText(ActionChip, 'TD'));
      await tester.pump();
      expect(tester.widget<TextField>(byKey('replyField')).controller!.text, 'TD');
      expect(rig.server.called('POST', '/api/v1/outbound'), isEmpty);
    });

    testWidgets('a refusal is shown and the text is kept for a retry', (tester) async {
      final rig = Rig();
      rig.server.routes['POST /api/v1/outbound'] = (_) => (429, {'error': 'rate_limited'});
      await rig.pump(tester);
      await openThread(tester);
      await tester.enterText(byKey('replyField'), 'TD');
      await tester.pump();
    ...
    testWidgets('the phone policy blocks the box and says whose policy it is', (tester) async {
      final rig = Rig(phone: {'effectivePolicy': 'off', 'sendPolicy': 'reply', 'phoneSendPolicy': 'off'});
      await rig.pump(tester);
      await openThread(tester);
    ...
    testWidgets('a client without the reply permission cannot reply', (tester) async {
    ...
    testWidgets('an offline phone is announced and the send tasks show how far they got, with cancel for queued ones', (tester) async {
      final rig = Rig(phone: {'online': false});
      rig.server.routes['GET /api/v1/outbound'] = (_) => {
            'items': [taskJson(id: 't_1'), taskJson(id: 't_2', body: 'Y', status: 'failed', error: 'recipient_not_recent')],
          };
      rig.server.routes['POST /api/v1/outbound/t_1/cancel'] = (_) => taskJson(id: 't_1', status: 'failed', error: 'cancelled');
      await rig.pump(tester);
      await openThread(tester);
      ...
      await tester.pumpAndSettle();
      expect(rig.server.called('POST', '/api/v1/outbound/t_1/cancel'), hasLength(1));
    });
  });
  ...
  group('new message', () {
    testWidgets('picks the phone and SIM, sends, and closes', (tester) async {
      final rig = Rig();
      rig.server.routes['POST /api/v1/outbound'] = (_) => (201, taskJson(mode: 'new'));
      await rig.pump(tester);
      ...
      await tester.pumpAndSettle();
      expect(rig.server.called('POST', '/api/v1/outbound').single.body, {'deviceId': 'd1', 'simSlot': 2, 'to': '139 0000 0000', 'body': '你好'});
      expect(byKey('newTo'), findsNothing, reason: 'the dialog closes once the task is accepted');
    });

    testWidgets('a phone that only allows replies blocks a new message', (tester) async {
      final rig = Rig(phone: {'effectivePolicy': 'reply', 'sendPolicy': 'reply'});
      await rig.pump(tester);
      await tester.tap(byKey('newMessage'));
      await tester.pumpAndSettle();
    ...
    testWidgets('a client without the send permission is told so', (tester) async {
  ...
  group('settings', () {
    ...
    testWidgets('shows the connection, and the connection test answers in words', (tester) async {
      final rig = Rig();
      await rig.pump(tester);
    ...
    testWidgets('changing the address verifies it first and a wrong one is refused', (tester) async {
      final rig = Rig();
      await rig.pump(tester);
      await openSettings(tester);
      rig.server.routes['GET /api/v1/me'] = (_) => (502, '');
      await tester.enterText(byKey('settingsUrl'), 'https://typo.example.com');
      await tester.pump();
    ...
    testWidgets('signing out asks first, then returns to the connect form and keeps the address', (tester) async {
      final rig = Rig();
      await rig.pump(tester);
      await openSettings(tester);
      await tester.ensureVisible(byKey('signOut'));
      await tester.pumpAndSettle();
