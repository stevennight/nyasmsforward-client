...
class ApiException implements Exception {
  ...
  ConnectionVerdict get verdict {
    if (kind == FailureKind.notOurServer) return ConnectionVerdict.addressSuspect;
  ...
  bool get isDenied => status != null && code != null && (status == 403 || status == 409 || status == 422 || status == 429 || status == 400);
  ...
  String get text {
    ...
    return switch (kind) {
      ...
      FailureKind.notOurServer => '这个地址不像 NyaSmsForward 服务器，请检查地址（是否需要 https://，或被反向代理拦截）',
      FailureKind.http when status == 429 => '请求太频繁，请稍后再试',
      FailureKind.http when status != null && status! >= 500 => '服务器暂时不可用（HTTP $status），请稍后再试',
      FailureKind.http when verdict == ConnectionVerdict.addressSuspect => '这个地址不像 NyaSmsForward 服务器，请检查地址（HTTP $status）',
      _ => '服务器拒绝了请求${code != null ? '（$code）' : status != null ? '（HTTP $status）' : ''}',
    };
  }
}

class ClaimResult {
  const ClaimResult({required this.deviceId, required this.token, required this.scopes});
...
class ApiClient {
  ApiClient({required String baseUrl, this.token, http.Client? httpClient, this.timeout = const Duration(seconds: 15)})
  ...
  Map<String, String> get _headers => {
        ...
        if (token != null) 'Authorization': 'Bearer $token', // only ever in this header, never in the URL
      };

  // --- pairing -----------------------------------------------------------------------------------------------

  Future<ClaimResult> claim({required String code, required String name, required String platform, required String appVersion}) async {
    final j = await _send('POST', '/api/v1/pair/claim', body: {'code': code, 'name': name, 'kind': 'client', 'platform': platform, 'appVersion': appVersion});
    return _claimResult(j);
  }

  /// Exchanges the admin password for a device token (docs/协议.md §3.1). The password is not kept anywhere.
  Future<ClaimResult> loginDevice({required String password, required String name, required String platform, required String appVersion, String? totp}) async {
    final j = await _send('POST', '/api/v1/auth/login-device', body: {
      'password': password,
      'name': name,
      'kind': 'client',
      'platform': platform,
  ...
  Future<({List<Conversation> items, int unread})> conversations({String? query, String? deviceId, bool unreadOnly = false}) async {
    final j = await _send('GET', '/api/v1/conversations', query: {'q': query, 'device': deviceId, 'unread': unreadOnly ? '1' : null, 'limit': '200'});
    return (
      items: [for (final c in (j['items'] as List?) ?? const []) Conversation.fromJson((c as Map).cast<String, Object?>())],
      unread: (j['unread'] as num?)?.toInt() ?? 0,
    );
  }

  Future<List<Message>> thread(String deviceId, String peerKey) async {
    final j = await _send('GET', '/api/v1/messages', query: {'device': deviceId, 'peer': peerKey, 'limit': '200'});
    final items = [for (final m in (j['items'] as List?) ?? const []) Message.fromJson((m as Map).cast<String, Object?>())];
    items.sort((a, b) => a.deviceTime != b.deviceTime ? a.deviceTime.compareTo(b.deviceTime) : a.id.compareTo(b.id));
    return items;
  }
  ...
  Future<List<int>> markThreadRead(String deviceId, String peer) async {
    final j = await _send('POST', '/api/v1/messages/read', body: {'deviceId': deviceId, 'peer': peer});
    return [for (final i in (j['changed'] as List?) ?? const []) (i as num).toInt()];
  }
  ...
  Future<OutboundTask> reply(int messageId, String body) async =>
      OutboundTask.fromJson(await _send('POST', '/api/v1/outbound', body: {'replyToMessageId': messageId, 'body': body}));

  Future<OutboundTask> sendNew({required String deviceId, int? simSlot, required String to, required String body}) async =>
      OutboundTask.fromJson(await _send('POST', '/api/v1/outbound', body: {'deviceId': deviceId, 'simSlot': ?simSlot, 'to': to, 'body': body}));

  Future<List<OutboundTask>> outbound({String? deviceId, String? peer, int limit = 5}) async {
    final j = await _send('GET', '/api/v1/outbound', query: {'device': deviceId, 'peer': peer, 'limit': '$limit'});
    return [for (final t in (j['items'] as List?) ?? const []) OutboundTask.fromJson((t as Map).cast<String, Object?>())];
  }

  Future<OutboundTask> cancelOutbound(String taskId) async =>
      OutboundTask.fromJson(await _send('POST', '/api/v1/outbound/$taskId/cancel'));
  ...
  Future<http.StreamedResponse> openEvents({String? lastEventId}) async {
    final request = http.Request('GET', Uri.parse('$baseUrl/api/v1/events'))
      ..headers.addAll({..._headers, 'Accept': 'text/event-stream', 'Cache-Control': 'no-cache', 'Last-Event-ID': ?lastEventId});
    try {
  ...
  Future<Map<String, Object?>> _send(String method, String path, {Map<String, Object?>? body, Map<String, String?>? query}) async {
    final params = {for (final e in (query ?? const <String, String?>{}).entries) if (e.value != null && e.value!.isNotEmpty) e.key: e.value!};
    ...
    try {
      ...
      if (response.statusCode >= 300 && response.statusCode < 400) {
        ...
        throw ApiException(kind: FailureKind.notOurServer, status: response.statusCode, message: 'redirect');
      }
      if (response.statusCode >= 400) throw _httpFailure(response.statusCode, text);
      ...
      if (decoded is! Map) throw ApiException(kind: FailureKind.notOurServer, status: response.statusCode, message: 'unexpected body');
      return decoded.cast<String, Object?>();
    } on ApiException {
      rethrow;
    } on FormatException {
      throw ApiException(kind: FailureKind.notOurServer, status: 200, message: 'unexpected body');
    } on Object catch (e) {
      throw _transportFailure(e);
    }
  }
  ...
  ApiException _httpFailure(int status, String text) {
    ...
    return ApiException(kind: FailureKind.http, status: status, code: code, message: message);
  }

  ApiException _transportFailure(Object e) => switch (e) {
        TimeoutException() => ApiException(kind: FailureKind.timeout, message: '$e'),
        HandshakeException() || TlsException() => ApiException(kind: FailureKind.tls, message: '$e'),
