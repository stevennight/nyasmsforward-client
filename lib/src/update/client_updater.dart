...
const _releaseApi = 'https://api.github.com/repos/$updateRepository/releases/latest';
const _downloadPrefix = 'https://github.com/$updateRepository/releases/download/';
...
class ClientUpdateService {
  ...
  final bool _windows;
  final bool _android;
  static const _installChannel = MethodChannel('app.nya.smsforward.client/updater');
  ...
  Future<ClientUpdateResult> check(String currentVersion) async {
    ...
    try {
      ...
      if (response.bodyBytes.length > _maxMetadataBytes) return const ClientUpdateFailed('Release 信息过大');
      final release = jsonDecode(utf8.decode(response.bodyBytes));
      if (release is! Map<String, dynamic>) return const ClientUpdateFailed('GitHub 返回的 Release 格式无效');
      if (release['draft'] == true || release['prerelease'] == true) {
        return const ClientUpdateFailed('GitHub 返回了草稿或预发行版本');
      }
      final tag = release['tag_name'] as String?;
      final latest = tag == null ? null : StableClientVersion.parse(tag.startsWith('v') ? tag.substring(1) : '');
      if (tag == null || latest == null) return ClientUpdateFailed('Release 标签不是稳定版本：${tag ?? ''}');
      if (latest.compareTo(current) <= 0) return ClientUpdateUpToDate(currentVersion);
      ...
      final assets = (release['assets'] as List?)
              ?.whereType<Map>()
              .map((asset) => _ReleaseAsset.fromJson(asset.cast<String, Object?>()))
              .toList() ??
          const <_ReleaseAsset>[];
      final artifact = assets.where((asset) => asset.name == artifactName).singleOrNull;
      final checksum = assets.where((asset) => asset.name == checksumName).singleOrNull;
      if (artifact == null) return ClientUpdateFailed('最新 Release 没有兼容的安装包：$artifactName');
      if (checksum == null) return ClientUpdateFailed('最新 Release 没有校验文件：$checksumName');
      if (!_trustedAsset(artifact, tag, artifactName) || !_trustedAsset(checksum, tag, checksumName)) {
        return const ClientUpdateFailed('Release 资产地址不属于受信任的仓库');
      }
      if (artifact.size <= 0 || artifact.size > _maxArtifactBytes) return const ClientUpdateFailed('安装包大小不符合安全限制');
  ...
  Future<ClientInstallResult> downloadAndInstall(ClientUpdate update) async {
    try {
      ...
      final checksumResponse = await _client
          .get(Uri.parse(update.checksumUrl), headers: const {'User-Agent': 'NyaSmsForward-Client-Updater'})
          .timeout(const Duration(seconds: 20));
      if (checksumResponse.statusCode != 200) throw FormatException('下载校验文件失败：HTTP ${checksumResponse.statusCode}');
      if (checksumResponse.bodyBytes.length > _maxChecksumBytes) throw const FormatException('校验文件过大');
      final expected = _checksumFor(utf8.decode(checksumResponse.bodyBytes), update.artifactName);
      if (expected == null) throw FormatException('校验文件中没有找到 ${update.artifactName}');

      final directory = Directory('${Directory.systemTemp.path}${Platform.pathSeparator}nyasmsforward-updates${Platform.pathSeparator}${update.version}')
        ..createSync(recursive: true);
      final artifact = File('${directory.path}${Platform.pathSeparator}${update.artifactName}');
      final response = await _client.send(
        http.Request('GET', Uri.parse(update.artifactUrl))..headers['User-Agent'] = 'NyaSmsForward-Client-Updater',
      ).timeout(const Duration(minutes: 3));
      if (response.statusCode != 200) throw FormatException('下载安装包失败：HTTP ${response.statusCode}');
      if (response.contentLength != null && (response.contentLength! <= 0 || response.contentLength! > _maxArtifactBytes)) {
      ...
      if (_windows) {
        await Process.start(artifact.path, const [], mode: ProcessStartMode.detached);
        exit(0);
      }
      if (_android) {
        try {
          await _installChannel.invokeMethod<void>('installApk', {'path': artifact.path});
          return const ClientInstallStarted();
        } on PlatformException catch (e) {
          if (e.code == 'permission_required') return const ClientInstallPermissionRequired();
...
class _ReleaseAsset {
  const _ReleaseAsset({required this.name, required this.url, required this.size});
...
class StableClientVersion implements Comparable<StableClientVersion> {
  ...
  factory StableClientVersion._fromMatch(RegExpMatch match) => StableClientVersion(
  ...
  static StableClientVersion? parse(String value) {
    final match = RegExp(r'^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$').firstMatch(value);
