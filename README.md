...
- **回复、新发与回收**：回复框（Enter 发送、Shift+Enter 换行、输入法组字时的 Enter 不发送）、快捷回复（点一下填入，仍需确认发送，可在设置里改）、发送任务状态条（排队 → 已下发 → 已发送 → 已送达 / 失败原因，排队中可取消）、新短信对话框（选手机、SIM、号码）；可多选短信移到回收站、30 天内恢复；权限 / 策略不足时说明是哪一层不允许。
